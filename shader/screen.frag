...
uniform sampler2D framePosition, frameColor, scene;
...
void main() {
	...
	// color = mix(color, (texture2D(frameColor, p-vec2(1,0))), step(p.x-1., 1.) * step(p.y, 1.));
	// color = mix(color, (texture2D(framePosition, p)), step(p.x, 1.) * step(p.y, 1.));
